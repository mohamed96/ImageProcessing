...
int main() {
    ...
    try {
        Image *img = readPPM("/home/mohamed/Scaricati/ppmimages/tree_1.ppm");
        std::cout << "Insert kernel dim: " << std::endl;
        std::cin >> n;
        RGBPixel *kernel=new RGBPixel[n*n];
        for(int i = 0; i<3; i++)
            for( int j=0; j<3; j++)
                kernel[i*3+j]=1;
        ...
        ImageProcessing<Image, RGBPixel> processing(img, kernel, n*n);
        processing.applyFilter();
        savePPM(*img, "/home/mohamed/Scaricati/ppmimages/tree1.ppm");
